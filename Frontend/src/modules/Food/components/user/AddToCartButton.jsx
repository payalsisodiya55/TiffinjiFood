...
export default function AddToCartButton({ item, className = "" }) {
  ...
  if (inCart) {
    return (
      <div className={`flex items-center gap-2 ${className}`} onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}>
        <div className="flex items-center gap-1 bg-[#D51F10] text-white rounded-md shadow-sm">
          <Button
            variant="ghost"
            size="icon"
            className="h-8 w-6 text-white hover:bg-[#b01a0d] hover:text-white"
  ...
  return (
    <Button
      ...
      className="bg-[#D51F10] hover:bg-[#b01a0d] text-white font-bold shadow-md transition-all active:scale-95"
    >
      Add to Cart
    </Button>
  )
